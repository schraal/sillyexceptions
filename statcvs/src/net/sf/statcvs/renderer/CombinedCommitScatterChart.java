...
public class CombinedCommitScatterChart extends Chart{
	 ...
	 * saves it as PNG
	 * @param commitScatterSeries the LOC history
	...
	private void createScatterChart(TimeSeries series_all, Map authorSeriesMap, String title) {
		...
		XYItemRenderer renderer = new PointXYRenderer(StandardXYItemRenderer.SHAPES,
			new StandardXYItemLabelGenerator(DateFormat.getInstance()));
		...
		while (authorsIt.hasNext()) {
			...
			XYItemRenderer renderer1 = new PointXYRenderer(StandardXYItemRenderer.SHAPES,
				new StandardXYItemLabelGenerator(DateFormat.getInstance()));
